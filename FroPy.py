    ...
    def draw(self, screen, mouse_x, mouse_y, style=1, font='Arial', text_color=(0, 0, 0)):
        ...
        if style == 1:
            ...
            if list_size[1] != 5 or list_size[1] != 0:
                pygame.draw.line(screen, (25, 25, 25), [self.x - 2, self.y - 2], [self.x + self.size - 1, self.y - 2],
                                 2)
                pygame.draw.line(screen, (25, 25, 25), [self.x - 2, self.y - 2], [self.x - 2, self.y + self.size / 2],
                                 2)
                ...
                pygame.draw.line(screen, (25, 25, 25), [self.x - 2, self.y + self.size / 2],
                                 [self.x + self.size - 1, self.y + self.size / 2], 2)
            else:
                pygame.draw.line(screen, (25, 25, 25), [self.x - 2, self.y - 2], [self.x + self.size + 1, self.y - 2],
                                 2)
                pygame.draw.line(screen, (25, 25, 25), [self.x - 2, self.y - 2], [self.x - 2, self.y + self.size / 2],
                                 2)
                pygame.draw.line(screen, (25, 25, 25), [self.x + self.size, self.y],
                                 [self.x + self.size, self.y + self.size / 2 + 1], 2)
                pygame.draw.line(screen, (25, 25, 25), [self.x - 2, self.y + self.size / 2],
                                 [self.x + self.size, self.y + self.size / 2], 2)

            if self.state == True:
                pygame.draw.rect(screen, (255, 255, 255),
                                 ((self.x + self.size / 2 - 2), self.y, self.size / 2, self.size / 2), 0)
            else:
        ...
        if self.text != '':
            ...
            if self.text_pos == 1:
                screen.blit(text,
                            (self.x - (text.get_width() + 10), self.y + ((self.size / 2) / 2 - text.get_height() / 2)))
...
class GroupBox:
    ...
    def draw(self, screen, mouse_x, mouse_y, outl_color=(0, 0, 0), bg_color=(25, 25, 25), text_color=(255, 255, 255),
             outl_thickness=5):
        ...
        pygame.draw.line(screen, outl_color, [self.x, self.y], [(self.x + self.width) + (outl_thickness / 2), self.y],
                         outl_thickness)
        pygame.draw.line(screen, outl_color, [self.x, self.y + 1 - (outl_thickness / 2)],
                         [self.x, (self.y + self.height) + (outl_thickness / 2)], outl_thickness)
        pygame.draw.line(screen, outl_color, [self.x, (self.y + self.height)],
                         [(self.x + self.width) + (outl_thickness / 2), (self.y + self.height)], outl_thickness)
        pygame.draw.line(screen, outl_color, [(self.x + self.width), self.y],
                         [(self.x + self.width), (self.y + self.height) + (outl_thickness / 2)], outl_thickness)

        if self.text != '':
            text = scale_text((self.width, (self.height * 0.15)), self.text, text_color)
            # text = font.render(self.text, 1, text_color)
            if self.text != "":
                screen.blit(text, (self.x + (self.width / 2 - text.get_width() / 2), self.y + (self.height * 0.02)))
                # draw title line
                pygame.draw.line(screen, outl_color, [self.x, self.y + text.get_height() + (self.height * 0.05)],
                                 [(self.x + self.width), self.y + text.get_height() + (self.height * 0.05)],
                                 outl_thickness
                                 )
    ...
    def draw(self, screen, mouse_x, mouse_y, edge, font="Arial", placeholder_color=(75, 75, 75)):
        ...
        if self.capture_text == True:
            ...
            if len(list(self.text)) > 0:
                ...
                screen.blit(scaled_text, (self.x + (self.width / 2 - scaled_text.get_width() / 2),
                                          self.y + (self.height / 2 - scaled_text.get_height() / 2)))
        else:
