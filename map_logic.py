...
def screen_Grid(screen, columns, rows,_Multiplier):
    ...
    upper_Left_Y = screen.get_height() - (((rows + 2) * height) + ((rows + 2) * margin))
    ...
    pygame.draw.rect(screen, (WHITE), ((upper_Left_X, upper_Left_Y), (lower_Right_X - (height //2) - (margin * 3), lower_Right_Y - (width //2))))
    ...
    for row in range(rows):
        for column in range(columns):
            ...
            if DEBUG:
                print("Upper x coord")
                print(upper_Left_X + (column * width) + (column * margin))
                print("Upper y coord")
                print(upper_Left_Y)
                print("Lower x coord")
                print((upper_Left_X + (column * width) + (column * margin)))
                ...
                print(lower_Left_Y)
                print("other y is")
                print((upper_Right_X, upper_Right_Y + (row * height) + (row * margin)))
...
def generate_Map(screen, _Multiplier, map_Sprite_Group):
    map_Grid = [
...
def main(screen, _Multiplier, map_Sprite_Group):



    pygame.display.set_caption('WW-TS')
...
if __name__ == '__main__':
    main(screen,_Multiplier, map_Sprite_Group)