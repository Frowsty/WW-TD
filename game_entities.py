...
class Player(pygame.sprite.Sprite):
    ...
    def actions(self, screen, get_key, auto_reload):
        ...
        if pygame.key.get_pressed()[pygame.K_r] or self.reloading == True or (
                auto_reload == True and len(self.bullets) == 5):
            self.reloading = True
        ...
        if pygame.key.get_pressed()[pygame.K_SPACE] and (pygame.time.get_ticks() - self.fired_tick) >= 500:
            ...
            if len(self.bullets) < self.ammo:
                ...
                if self.player_facing == 0:
                    self.bullets.append(Projectile(round(self.cur_pos[0] + self.rect.get_width() / 1.6),
                                                   round(self.cur_pos[1] + self.rect.get_height() / 8),
                                                   self.player_facing))
                if self.player_facing == 1:
                    self.bullets.append(Projectile(round(self.cur_pos[0] + self.rect.get_width() / 8),
                                                   round(self.cur_pos[1] + self.rect.get_height() / 3.2),
                                                   self.player_facing))
                if self.player_facing == 2:
                    self.bullets.append(Projectile(round(self.cur_pos[0] + self.rect.get_width() / 3.3),
                                                   round(self.cur_pos[1] + self.rect.get_height() / 1.22),
                                                   self.player_facing))
                if self.player_facing == 3:
                    self.bullets.append(Projectile(round(self.cur_pos[0] + self.rect.get_width() / 1.22),
                                                   round(self.cur_pos[1] + self.rect.get_height() / 1.6),
                                                   self.player_facing))
